...
function configureServiceBinding(binding) {
    ...
    return new Promise((resolve, reject) => {
        ...
        ibmcloud.on('close', (code) => {
            if (code === 2) {
                const err = new this.serverless.classes.Error('Unable to execute `ibmcloud wsk service bind` command. Is IBM Cloud Functions CLI plugin installed?');
                return reject(err);
            }
            if (code > 0) {
...
function configureServiceBindings() {
    ...
    return Bluebird.all(bindings.packages.map(sbs => Bluebird.mapSeries(sbs, sb => configureServiceBinding.bind(this)(sb)))).then(() => Bluebird.all(bindings.fns.map(sb => configureServiceBinding.bind(this)(sb))));
}
exports.default = configureServiceBindings;
