...
function main(params) {
    ...
    return conversation.sendMessage("conInit" in params, params)
        .then(function (params) { // conversation succeeded
        ...
        }, function (reason) { // conversation error
            console.error("Conversation Error: " + reason);
            throw reason;
        })
        .then(function (response) { // dispatcher succeeded
            return {
                headers: {
                    'Access-Control-Allow-Origin': '*',
                    'Content-Type': 'text/plain'
        ...
        }, function (reason) { // dispatcher error
